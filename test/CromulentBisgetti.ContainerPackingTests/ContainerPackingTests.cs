	...
	public class ContainerPackingTests
	{
		...
		public void EB_AFIT_Passes_700_Standard_Reference_Tests()
		{
			...
			using (Stream stream = assembly.GetManifestResourceStream(resourceName))
			{
				using (StreamReader reader = new StreamReader(stream))
				{
					...
					List<(List<Item> ItemsToPack, string[] TestResults, string[] containerDims)> testData =
						new List<(List<Item> ItemsToPack, string[] TestResults, string[] containerDims)>();
					while (reader.ReadLine() != null && counter <= 700)
					{
						testData.Add(ReadTestData(reader));
						counter++;
					}

					Parallel.ForEach(testData, td => PackAndAssert(td.containerDims, td.ItemsToPack, td.TestResults));
				}
			}
		}
		...
		private static void PackAndAssert(string[] containerDims, List<Item> itemsToPack, string[] testResults)
		{
			...
			List<ContainerPackingResult> result =
				PackingService.Pack(containers, itemsToPack, new List<int> { (int)AlgorithmType.EB_AFIT });
		...
		private static (List<Item> ItemsToPack, string[] TestResults, string[] containerDims) ReadTestData(StreamReader reader)
		{
			...
			for (int i = 0; i < itemTypeCount; i++)
			{
				...
				Item item = new Item(0, Convert.ToDecimal(itemArray[1]), Convert.ToDecimal(itemArray[3]),
					Convert.ToDecimal(itemArray[5]), Convert.ToInt32(itemArray[7]));
