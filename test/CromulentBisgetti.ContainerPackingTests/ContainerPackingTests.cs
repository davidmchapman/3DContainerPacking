                ...
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    while (reader.ReadLine() != null && counter <= 700)
                    {
                        ...
                        Assert.AreEqual(
                            ...
                            result[0].AlgorithmPackingResults[0].UnpackedItems.Count,
                            Convert.ToDecimal(testResults[1]));

                        // Assert that the number of items successfully packed equals the number stated in the published reference.
                        Assert.AreEqual(
                            result[0].AlgorithmPackingResults[0].PackedItems.Count,
                            Convert.ToDecimal(testResults[2]));
                        ...
                        Assert.IsTrue(result[0].AlgorithmPackingResults[0].PercentContainerVolumePacked ==
                        ...
                        Assert.AreEqual(
                            result[0].AlgorithmPackingResults[0].PercentItemVolumePacked,
